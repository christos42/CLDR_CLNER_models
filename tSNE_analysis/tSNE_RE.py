...
class tSNE_analysis:
    ...
    def create_plots(self):
        for n in [1, 5, 10, 20]:
            ...
            embeddings_selected = [self.embeddings[i] for i in sampled_indexes]
            labels_selected = [self.labels[i] for i in sampled_indexes]

            X_embedded = TSNE(n_components=2).fit_transform(embeddings_selected)

            sns.set(rc={'figure.figsize': (11.7, 8.27)})

            palette = sns.color_palette("bright", 2)

            sns.scatterplot(X_embedded[:, 0], X_embedded[:, 1], hue=labels_selected, legend='full',
                            palette=palette)

            if not os.path.exists('plots/split_' + str(self.split_num) + '/RE/'):
                os.makedirs('plots/split_' + str(self.split_num) + '/RE/')
            plt.savefig('plots/split_' + str(self.split_num) + '/RE/'+ 'hard_' + str(n * len(self.indexes_1)) + '.png')
